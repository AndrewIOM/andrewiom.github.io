...
var svgWidth = $('#gcm-graph').width();
console.log(svgWidth);
var svgHeight = svgWidth * 0.75;
...
function makeCharts(data) {
    ...
    var svg = d3.select('#gcm-graph').append('svg')
        .attr('width', svgWidth)
   ...
   var g1 = svg
    ...
    var rectClip = g1.append('clipPath')
        .attr('id', 'rect-clip')
        .append('rect')
        .attr('width', 0)
        .attr('height', chartHeight);

    drawPaths(g1, data, x, y);
    addAxesAndLegend(g1, xAxis, yAxis, margin, chartWidth, chartHeight);
    startTransitions(g1, chartWidth, chartHeight, rectClip, x);

    d3.json('/projects/gcm-graphs/data/barjuly.json', function (error, rawData) {
        if (error) {
            console.error(error);
            return;
        }
...
function makeLegend(svg) {
    ...
    legend.append('rect').attr('class', 'rcp45-line').attr('width', 50).attr('height', 1).attr('transform', 'translate(0,100)');    
    legend.append('rect').attr('class', 'rcp26-line').attr('width', 50).attr('height', 1).attr('transform', 'translate(0,125)');
    ...
    legend.append('text').text('Median of All Models for Medium-High Emissions').attr('transform', 'translate(150,75)');    
    legend.append('text').text('Median of All Models for Medium-Low Emissions').attr('transform', 'translate(150,100)');    
    legend.append('text').text('Median of All Models for Low Emissions').attr('transform', 'translate(150,125)');
...
d3.json('/projects/gcm-graphs/data/linejuly.json', function (error, rawData) {
    if (error) {
        console.error(error);
        return;
    }
